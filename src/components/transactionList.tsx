...
export default function TransactionsList({
  ...
  const sendTransaction = async (transaction: TransactionWithSignatures) => {
    try {
      ...
      // Get the built UserOperation
      const finalUserOp = builder.getOp();

      // Send UserOperation to the EntryPoint contract
      const tx = await entryPointContract.handleOps([finalUserOp], address, {
        gasLimit: ethers.BigNumber.from(finalUserOp.callGasLimit).add(
